...
Page({
	data: {
		...
		isFirstLoadAllStandard: ['getMainData', 'getUserInfoData','userCouponGet','getUserData'],
		pay: {
			coupon: []
		},
		couponTotalPrice: 0,
	...
	onShow() {
		...
		}else if (wx.getStorageSync('info').parent_no != '') {
			getApp().globalData.user_no = wx.getStorageSync('info').parent_no;
			self.getParentStoreData();
		};
	...
	pay(order_id) {
		...
		if (self.data.parentStoreData.user_no) {
		...
		if (self.data.parentStoreData.partner && self.data.parentStoreData.partner.parent_no != '') {
			if (self.data.pay.wxPay && self.data.pay.wxPay.price && self.data.pay.wxPay.price > 0 && self.data.mainData.shop_ratio >
				0) {
				postData.payAfter.push({
					tableName: 'FlowLog',
	...
	useCoupon(e){
	  ...
	  if(findItem){
	    ...
	    if(findCoupon.type==1){
	      ...
	      var couponPrice = parseFloat(self.data.price).toFixed(2) - parseFloat(findCoupon.discount/100*self.data.price).toFixed(2);
	    };
